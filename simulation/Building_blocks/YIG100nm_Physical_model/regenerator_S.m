...
gain_out = 1.2;    % output amplifier gain
h=30;               % thinckness  [nm]
w=100;              % width  [nm]
L3=1130;            % length of the coupling region of the first DC [nm]
L4=410;             % length of the coupling region of the second DC [nm]
...
if Lc_avg_flag == 1
    ...
    fprintf('\n RegS: for the first DC, with average Lc_avg = %dnm, Pout1/(Pout1+Pout2) = %d%% \n',Lc_avg1,pow_par1*100)
    fprintf('\n RegS: for the second DC, with average Lc_avg = %dnm, Pout1/(Pout1+Pout2) = %d%% \n',Lc_avg,pow_par*100)
end
