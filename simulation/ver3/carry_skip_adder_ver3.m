function [X] = carry_skip_adder_ver3(A,B,carry,Nbit)
...
B2 = zeros(Nbit,N_inf);
for i=1:Nbit
    [A1(i,:),A2(i,:)] = duplicator_ver3(A(i,:));
    [B1(i,:),B2(i,:)] = duplicator_ver3(B(i,:));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:Nbit
    p(i,:) = XOR_ver3( A1(i,:), B1(i,:) );
end
P_block = zeros(N_block,N_inf); % propagate of every block
for i=1:N_block
    P_block(i,:) = AND4_ver3( p((i-1)*4+1,:), p((i-1)*4+2,:), p((i-1)*4+3,:), p((i-1)*4+4,:) );
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if rem(Nbit,4) == 0
    for i=N_block:-1:1 % for every block
        RCA_out = RCA_Nbit_ver3( A2((i-1)*4+1:(i-1)*4+4, :), B2((i-1)*4+1:(i-1)*4+4, :), carry, 4 ); % 4 is the number of FA of every RCA group
        X( (i-1)*4+2:(i-1)*4+5, :) = RCA_out(2:end, :);
        carry = mux2to1_ver3(RCA_out(1,:), carry, P_block(i,:));
    end
    X(1,:) = carry;
else
